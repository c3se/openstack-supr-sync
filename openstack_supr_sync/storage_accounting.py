...
snapshots = [dict(project=projects_lookup[s.metadata.get('owner_id', '')], size=s.size // (1024 ** 3))
          for s in openstack_objects.get_vm_snapshots() if s.metadata.get('owner_id', '') in projects_lookup]
...
for p in projects:
    ...
    backup_size = sum([s['size'] for s in backup if p.name == s['project']])
    volume_size = sum([s['size'] for s in volumes if p.name == s['project']])
